...
def compute_targetwise_metrics(epoch_results, target_name):
...
def compute_metrics(epoch_results, target_names):
    metrics = {
        target_name: compute_targetwise_metrics(epoch_results, target_name)
        for target_name in target_names
    }
    metrics["loss"] = epoch_results["running_loss"]["loss"]
    metrics["epoch_acc"] = np.mean(
        [metrics[target_name]["epoch_acc"] for target_name in target_names]
    )
    return metrics
...
def log_targetwise_metrics(
    ...
    experiment.log_metric(
        f"{target_name} Average epoch {fold} loss", epoch_loss, epoch=epoch, step=epoch
    )
    if n_classes > 2:
        ...
        experiment.log_metric(
            f"{target_name} {fold} ROC AUC", np.mean(roc_auc), epoch=epoch, step=epoch
        )
    else:
        experiment.log_metric(
            f"{target_name} {fold} ROC AUC", roc_auc, epoch=epoch, step=epoch
        )
    experiment.log_metric(
        f"{target_name} {fold} balanced accuracy", acc, epoch=epoch, step=epoch
    )


def log_metrics(experiment, target_names, label_names, epoch, metrics, fold="Train"):
    for target_name in target_names:
    ...
    experiment.log_metric(
        f"{fold} loss", np.mean(metrics["loss"]), epoch=epoch, step=epoch
    )
    experiment.log_metric(
        f"{fold} balanced accuracy", metrics["epoch_acc"], epoch=epoch, step=epoch
    )


def log_confusion_matrices(
    experiment, target_names, label_names, epoch, results, fold="Validation"
):
    for target_name in target_names:
