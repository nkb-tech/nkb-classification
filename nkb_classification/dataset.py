...
class InferDataset(Dataset):
    def __init__(
        self, folder_path, train_annotations_file, target_names, transform=None
    ):
        super(
            InferDataset,
            self,
        ...
        self.classes = {
            target_name: np.sort(np.unique(self.train_ann_table[target_name].values))
        ...
        self.imgs = [
            str(p) for p in self.folder.iterdir() if p.suffix.lower() in self.ext
        ]
...
class GroupsDataset(Dataset):
    def __init__(self, root, ann_file, dict_path, transform=None):
        super().__init__()
...
class AnnotatedMultitargetDataset(Dataset):
    ...
    def __init__(self, annotations_file, target_names, fold="test", transform=None):
        ...
        self.target_names = [*sorted(target_names)]
        self.classes = {
            target_name: np.sort(np.unique(self.table[target_name].values))
            for target_name in self.target_names
        }
        self.class_to_idx = {
            target_name: {k: i for i, k in enumerate(classes)}
        ...
        labels = {
            target_name: np.array(
                self.class_to_idx[target_name][
                    self.table.iloc[idx, self.table.columns.get_loc(target_name)]
                ],
                dtype=np.int64,
            )
            for target_name in self.target_names
...
def get_dataset(data, pipeline):
    ...
    if data["type"] == "GroupsDataset":
        dataset = GroupsDataset(
            data["root"], data["ann_file"], data["group_dict"], transform=transform
        )
    elif data["type"] == "AnnotatedMultitargetDataset":
        dataset = AnnotatedMultitargetDataset(
            data["ann_file"], data["target_names"], data["fold"], transform=transform
        )
    else:
